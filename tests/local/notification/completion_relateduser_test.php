...
/**
 * Program completion notification for related users test.
 *
 * @group      openlms
 ...
 *
 * @covers \enrol_programs\local\notification\completion_relateduser
 */
final class completion_relateduser_test extends \advanced_testcase {
    ...
    public function test_notify_users() {
        ...
        $now = time();
        $allocation1->timecompleted = $now - completion_relateduser::TIME_CUTOFF + 100;
        $DB->update_record('enrol_programs_allocations', $allocation1);
        $allocation2->timecompleted = $now - completion_relateduser::TIME_CUTOFF - 100;
        ...
        $generator->create_program_notification(['notificationtype' => 'completion_relateduser', 'programid' => $program1->id]);
        $generator->create_program_notification(['notificationtype' => 'completion_relateduser', 'programid' => $program2->id]);

        $this->setCurrentTimeStart();
        $sink = $this->redirectMessages();
        completion_relateduser::notify_users($program1, null);
        $messages = $sink->get_messages();
        $sink->close();
        $this->assertCount(1, $messages);
        $message = $messages[0];
        $this->assertSame('User ' . fullname($user1). ' completed program', $message->subject);
        ...
        $this->assertSame('completion_relateduser_notification', $message->eventtype);
        $this->assertSame($program1->fullname, $message->contexturlname);
        $this->assertTimeCurrent(notification_manager::get_timenotified($user1->id, $program1->id, 'completion_relateduser'));
        $this->assertNull(notification_manager::get_timenotified($user2->id, $program1->id, 'completion_relateduser'));

        $this->setCurrentTimeStart();
        $sink = $this->redirectMessages();
        completion_relateduser::notify_users(null, $user1);
        $messages = $sink->get_messages();
        $sink->close();
        $this->assertCount(1, $messages);
        $message = $messages[0];
        ...
        $this->assertTimeCurrent(notification_manager::get_timenotified($user1->id, $program2->id, 'completion_relateduser'));
        $this->assertNull(notification_manager::get_timenotified($user2->id, $program2->id, 'completion_relateduser'));

        $this->setCurrentTimeStart();
        $sink = $this->redirectMessages();
        completion_relateduser::notify_users(null, null);
        $messages = $sink->get_messages();
        $sink->close();
        $this->assertCount(1, $messages);
        $message = $messages[0];
        $this->assertSame('User ' . fullname($user2). ' completed program', $message->subject);
        $this->assertSame('-10', $message->useridfrom);
        $this->assertSame($related2->id, $message->useridto);
        $this->assertSame('enrol_programs', $message->component);
        $this->assertSame('completion_relateduser_notification', $message->eventtype);
        $this->assertSame($program2->fullname, $message->contexturlname);
        $this->assertTimeCurrent(notification_manager::get_timenotified($user2->id, $program2->id, 'completion_relateduser'));
